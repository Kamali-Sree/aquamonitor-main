...
function createVersionedPages() {
    const pages = ['home', 'dashboard', 'analytics', 'predator', 'about'];
...
function processHTMLContent(htmlContent, currentPage) {
    ...
    let modifiedData = htmlContent
        ...
        .replace(/href="predator\.html"/g, `href="/${VERSIONED_PAGES.predator}"`)
...
app.get(`/${VERSIONED_PAGES.predator}`, (req, res) => {
    const filePath = path.join(__dirname, 'public', 'predator.html');
    fs.readFile(filePath, 'utf8', (err, data) => {
        if (err) return res.status(404).send('Page not found');
        res.set('Content-Type', 'text/html; charset=utf-8');
        res.send(processHTMLContent(data, 'predator'));
    });
});
...
app.get('/predator.html', (req, res) => {
    res.redirect(301, `/${VERSIONED_PAGES.predator}`);
});
...
app.post('/api/predict-predator', upload.single('image'), (req, res) => {
    ...
    const python = spawn('python', [
        path.join(__dirname, 'predict_predator.py'),
        imagePath
    ]);
    ...
    python.stderr.on('data', (data) => {
        ...
        if (!msg.includes('oneDNN') && !msg.includes('UserWarning') && !msg.includes('INFO:')) {
            errorOutput += msg;
            console.error(`❌ Predator Detection Error: ${msg}`);
        }
    });
    ...
    python.on('close', (code) => {
        ...
        console.log(`🦊 Predator detection process completed with code: ${code}`);
        ...
        try {
            const parsed = JSON.parse(result);
            console.log(`✅ Predator detection successful`);
            res.json(parsed);
        } catch (e) {
            console.error(`❌ Predator detection JSON parse error:`, e);
            res.status(500).json({ error: 'Detection failed', details: result });
        }
    });
});
